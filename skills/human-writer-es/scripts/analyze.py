#!/usr/bin/env python3
"""
human-writer-es / analyze.py — deterministic AI-tell detector (Spanish).

Specialization of the master human-writer analyzer for Spanish. Differences vs
the master (everything else is identical):
  - --lang choices=["es"].
  - detect_tricolons uses a Spanish conjunction (y|e).
  - NEW detect_inverted_marks: flags ?/!-terminated sentences missing the
    opening ¿/¡ (Spanish-only typography tell), wired as a low-weight branch.
...
# Any remaining HTML comment is metadata (provenance headers, editorial notes,
# tooling markers), never rendered prose. Its punctuation — notably the `!` in
# the `<!--` opener itself — must not reach the detectors, or it would phantom-fire
# the inverted-mark check. Applied AFTER the named ignore-region regex so that
# `human-writer:ignore-*` keeps its block-stripping semantics.
...
def count_sentences(text: str) -> int:
    # Spanish sentence terminators stay [.!?] — the inverted marks ¿¡ are
    # OPENERS, not terminators, so they must not split sentences.
...
def build_recommendations(tells: dict) -> list[dict]:
    ...
    inv = styl.get("inverted_marks")
    if inv and inv.get("flag"):
        recs.append({
            "priority": "medium" if inv.get("severity") == "high" else "low",
            "action": (
                f"Add missing inverted marks: {inv.get('value')} sentence(s) end in "
                "?/! without an opening ¿/¡. Insert the matching opener at the start "
                "of each interrogative/exclamative span."
            ),
        })
...
def compute_score(tells: dict, weights: dict) -> int:
    ...
    if tricolons >= 3:
        stylistic_pts += 5
    # NEW: inverted-marks typography tell — SMALL low-weight branch (Spanish).
    # Missing ¿/¡ is a real surface signature but is register-sensitive (chat
    # legitimately drops them), so it contributes only a few points and never
    # dominates the score.
    inv = styl.get("inverted_marks")
    if inv and inv.get("flag"):
        stylistic_pts += 5 if inv.get("severity") == "high" else 3
...
# Spanish tricolon: "X, Y y Z" / "X, Y, y Z" / "X, Y e Z" (the conjunction is
# `e` before an i-/hi- sound, otherwise `y`). Matches the comma-comma-conjunction
# rhythm the same way the master EN/FR detectors do.
TRICOLON_ES = re.compile(r"\b\w+,\s*\w+,?\s*(?:y|e)\s+\w+\b", flags=re.IGNORECASE)


def detect_tricolons(text: str) -> int:
    return len(TRICOLON_ES.findall(text))


# Split text into orthographic sentences for the inverted-mark check. We split
# on the terminators .!? but KEEP the terminator with its sentence so we can see
# whether the sentence ended in ? or !.
_SENTENCE_SPLIT_RX = re.compile(r"[^.!?]*[.!?]", flags=re.DOTALL)


def detect_inverted_marks(text: str, threshold: int) -> dict:
    """Spanish typography tell: a sentence ending in ? or ! must carry the
    matching opening ¿ or ¡ somewhere earlier in that sentence.

    For every ?/!-terminated sentence we check for the presence of *some* ¿ (for
    `?`) or ¡ (for `!`) earlier in the same sentence span. The opener does not
    have to be at the orthographic start — Spanish places it at the start of the
    interrogative/exclamative span ("Si quieres, ¿por dónde empiezas?"). Missing
    openers are counted; the detector flags when the count exceeds `threshold`.

    Calibrated NOT to fire on clean native prose (which always pairs the marks).
    """
    missing = 0
    total = 0
    for m in _SENTENCE_SPLIT_RX.finditer(text):
        chunk = m.group(0)
        terminator = chunk.rstrip()[-1] if chunk.rstrip() else ""
        if terminator == "?":
            total += 1
            if "¿" not in chunk:
                missing += 1
...
def analyze(text: str, lang: str, content_type: str) -> dict:
    ...
    inverted = detect_inverted_marks(text, thresholds["inverted_marks_missing_max"])
...
def render_human(result: dict) -> str:
    lines: list[str] = []
    lines.append("=" * 60)
    lines.append(f"  human-writer-es / analyze.py — {result.get('input_file', '<stdin>')}")
    lines.append("=" * 60)
    lines.append(
    ...
    inv = styl.get("inverted_marks") or {}
    inv_flag = "FLAG" if inv.get("flag") else "ok"
    lines.append(
        f"inverted ¿¡  : {inv.get('value', 0)} missing of "
        f"{inv.get('questions_exclamations', 0)} ?/! sentences  [{inv_flag}]"
    )
...
def main() -> int:
    p = argparse.ArgumentParser(description="Deterministic AI-tell analyzer (Spanish)")
    p.add_argument("--input", type=Path, help="Input file (markdown/txt). Stdin if omitted.")
    p.add_argument("--lang", choices=["es"], required=True)
